...
func main() {
	...
	if cmd.Config == "" {
		log.Error("config is null,exit...,please -h see help")
		os.Exit(-1)
	}
...
func DebugProfile() {
	...
	go func() {
		select {
		case sign := <-s:
			log.Warn("Get signal:%v, Profile File is cpu.prof/mem.prof", sign)
			//cpu
			pprof.StopCPUProfile()
			if err := cpu.Close(); err != nil {
				log.Error("cpu pprof file close err:%v", err)
			}
			//mem
			runtime.GC() // get up-to-date statistics
			if err := pprof.WriteHeapProfile(mem); err != nil {
				log.Error("could not write memory profile:%v", err)
			}
			if err := mem.Close(); err != nil {
				log.Error("mem pprof file close err:%v", err)
			}
			os.Exit(-1)
		}
	}()
	log.Warn("Starting loading performance data, please press CTRL+C exit...")
}
